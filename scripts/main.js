...
export class SoccerBettingGame {
        ...
        try {
            ...
            if (type === 'full-match') {
                try {
                    ...
                    // Resume game when full-match bet is placed (Requirement 4.2)
            ...
            // Ensure game resumes regardless of error
            try {
                if (type === 'action') {
    ...
    showInlineBetSlip(outcome, odds) {
        try {
            // Pause game for full-match betting opportunity (Requirement 4.1, 4.5)
            if (this.pauseManager && !this.pauseManager.isPaused()) {
                const pauseSuccess = this.pauseManager.pauseGame('FULL_MATCH_BETTING', 30000); // 30 second timeout
                if (pauseSuccess) {
                    console.log('SoccerBettingGame: Game paused for full-match betting opportunity');
                    this.addEventToFeed('⏸️ Game paused for betting opportunity', 'text-blue-400');
                } else {
                    console.warn('SoccerBettingGame: Failed to pause game for full-match betting');
                }
            }
            ...
            // Hide pause overlay if it's visible since betting interface should be accessible
            if (this.pauseUI && this.pauseUI.isOverlayVisible()) {
                this.pauseUI.hidePauseOverlay();
            }
            ...
            console.log(`SoccerBettingGame: Full-match betting slip shown for ${outcome}`);
        } catch (error) {
            console.error('Error showing inline bet slip:', error);
            // Continue without pause if there's an error
        }
    }
    ...
    hideInlineBetSlip() {
        try {
            ...
            // Resume game when full-match betting is cancelled/completed (Requirement 4.2)
            this.handleBettingDecisionComplete('full_match_cancelled');
            
            console.log('SoccerBettingGame: Full-match betting slip hidden, game resuming');
        } catch (error) {
